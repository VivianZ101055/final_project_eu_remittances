...
remittance_inflows <- remittance_inflows %>%
  pivot_longer(
    ...
    names_prefix = "yr") %>%
  clean_names %>%
  rename(remmitance_inflows=value, year_remittance=remittance_inflows, country_name = migrant_remittance_inflows_us_million) %>%
  select(-c(x2019e, remittances_as_a_share_of_gdp_in_2019_percent))

full_data <- full_data %>%
  inner_join(remittance_inflows, by = "country_name")
